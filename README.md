...
4. （42）接雨水（On 解法，用辅助数组，思考对于单一的桶，左右边界木板的最长长度，使用数组存储前缀最大值和后缀最大值；O1 解法，双指针，思路和 On 的一致，只是不需要存储前缀后缀值，直接使用双指针存储左右木板的最长长度，使用双指针的核心是**在移动指针时，每次总能确定左右两边其中一个桶能容纳多少水**）。
...
3.  (239) 滑动窗口的最大值（滑动窗口 + 最大堆，注意堆要同时存储值和下标；滑动窗口 + 双端单调递减队列，单调递增还是单调递减取决于存储最大值还是最小值，双端和存储下标都是为了保证删除左端入队的元素）。
4.  「编码比较复杂」【1】(76) 最小覆盖子串（滑动窗口 -> 本质是双指针，有两个关键的变量：match 和 windowMap，主要的逻辑包括了 **right 的 for 循环和 left 的 while 循环**）。
