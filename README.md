...
1. 【2】（1）两数之和（Map 存储；排序）。
2. 【2】（49）字母异位词分组（排序比较；字母计数）。
3. 【2】（128）最长连续序列（利用哈希表，从最小的开始算）。

## 2.双指针

1. 【2】（283）移动零（双指针，快指针找非 0，慢指针挪位置；单指针，非零压缩前移，最后补零）。
2. 【2】（11）盛水最多的容器（左右指针夹逼 + 贪心，比我高的我才计算）。
3. 「*」【2】（15）三数之和（排序，利用等于去重，左右指针夹逼 + 等于去重，有点二分查找的感觉）。
4. 【2】（42）接雨水（On 解法，用辅助数组，思考对于单一的桶，左右边界木板的最长长度，使用数组存储前缀最大值和后缀最大值；O1 解法，双指针，思路和 On 的一致，只是不需要存储前缀后缀值，直接使用双指针存储左右木板的最长长度，使用双指针的核心是**在移动指针时，每次总能确定左右两边其中一个桶能容纳多少水**）。

## 3.滑动窗口

1. 【2】（3）无重复字符的最长子串（滑动窗口，从字符集，也就是哈希集合的角度考虑问题，记录每个字符是否出现过，只要发现 set 里面有重复，那么就一直让左指针去移动）。
2. 【2】（438）找到字符串中所有字母异位词分组（滑动窗口，记录每个字母出现的次数，然后使用 `Arrays.equals` 进行比较）。

## 4.子串/子数组

1. 「*」【2】（560）和为 K 的子数组（前缀和，哈希表记录前缀和出现的次数，注意保存 0）。
2. 【2】（239）滑动窗口的最大值（滑动窗口 + 最大堆，注意堆要同时存储值和下标；滑动窗口 + 双端单调递减队列，单调递增还是单调递减取决于存储最大值还是最小值，双端和存储下标都是为了保证删除左端入队的元素）。
3. 「*」【2】(76) 最小覆盖子串（滑动窗口 -> 本质是双指针，有两个关键的变量：match 和 windowMap，主要的逻辑包括了 **right 的 for 循环和 left 的 while 循环**）。

## 5.普通数组

1. 【2】（53）最大子数组和（前缀和，找最小的前缀和，注意区分**当前**的前缀和与**过往**前缀和的最小值；动态规划，考虑 `nums[i]` 单独成为一段还是加入 `f(i−1)` 对应的那一段，主要是明白 `f[i]` 的含义，以 `nums[i]` 结尾的，如果考虑 `f[i] = max(f[i - 1] + nums[i], f[i - 1])` 的话，就不是以 `nums[i]` 结尾了）。

> 前缀和的关键词的是**连续**。

2. 【2】（56）合并区间（按照左端点排序，记着考虑 `[[1, 4], [2, 3]]` 的情况）。
3. 【2】（189）轮转数组（一维数组轮转；O(1) 空间复杂度的方法使用三次 reverse 即可解决）。
4. 【2】（238）除自身以外数组的乘积（前缀积，后缀积，左右开弓；O(1) 空间复杂度的方法可以利用 res 数组，后缀积需要单独定义一个变量 R）。 
5. 【2】（41）缺失的第一个正数（原地哈希，将 [1, N] 中的每个数放到对应的位置，然后遍历找到第一个不符合的位置，记得 if -> while）。

## 6.矩阵

1. 【2】（73）矩阵置零（用两个标记数组分别记录每一行和每一列是否有零出现；O(1) 的解法是使用输入 **matrix** 的第一行与第一列记录是否有零出现，然后额外使用两个标记变量分别记录第一行和第一列是否原本包含 0）。
2. 「*」【2】（54）螺旋矩阵（定义变量用来记录遍历了多少，定义变量用来记录是否遍历过，然后定义四个方向 `[row, col]` 进行循环；O(1) 的方法是使用四个变量来记录边界）。
3. 【2】（48）旋转图像（可以找到规律为：对于矩阵中第 i 行的第 j 个元素，在旋转后，它出现在第 j 行倒数第 i 列的位置，要注意赋值顺序，`[i][j]` 为逆时针，`[i][j]` 在后为顺时针；第二种方法是先上下翻转，然后主对角线翻转）。
4. 【2】（240）搜索二维矩阵 II（左下角或者右上角起手，根据大小定方向）。

## 7.链表（链表需要建模 + 细心）

1. 「*」【2】（160）相交链表（A + B = B + A，重点是两个链表要互相**换链**，一次只能走一步，因此需要 else）。
...
7. （2）两数相加（模拟加法，注意进位；空间复杂度为 O(1) 的解法是使用原链表）。

8. （19）删除链表的倒数第 N 个结点（双指针，引入 dummy 节点，方便删除头结点）。

9. 【2】（24）两两交换链表中的节点（递归，理解交互链表中的两个节点是什么意思；迭代可使用 dummy 节点，虽然要交换两个节点，但是需要三个 ListNode 变量）。

10. （138）随机链表的复制（哈希表，使用 hashmap 建立原始节点和新节点的映射，先 next，再 random；节点拆分，我们首先将该链表中每一个节点拆分为两个相连的节点，例如对于链表 A -> B -> C，我们可以将其拆分为 A -> A' -> B -> B' -> C -> C'。对于任意一个原节点 S，其拷贝节点 S' 即为其后继节点）。

11. （148）排序链表（归并排序，递归，需要合并两个有序链表子方法；迭代也可以完成，主要依靠 subLength，此处不使用该方法）。

12. 【2】（146）**LRU 缓存**（Map + 双向链表，注意事项：capacity、map、dummy、tail，注意 next 是否为 null 的情况，从而更新 tail，**tail 和链表节点中的 key 都是为了解决容量不足的问题**）。

13. （23）合并 k 个升序链表（优先队列，维护当前每个链表没有被合并的元素的最前面一个，k 个链表就最多有 k 个满足这样条件的元素，每次在这些元素里面选取 val 属性最小的元素合并到答案中。在选取最小元素的时候，我们可以用优先队列来优化这个过程）。

14. （25）k 个一组翻转链表（直接迭代来实现，需要一个反转链表的子函数，重点是弄清楚链表的衔接，反转链表中参数加 tail 和不加 tail（要就是被反转的链不断开或者断开）的终止条件不同，一个是 pre != tail，一个是 cur != null，需要注意）。
...
5. （543）二叉树的直径（递归，计算左右子树的深度，然后求最大值）。
...
14. （236）二叉树的最近公共祖先（递归，存储父节点，可以用哈希表存储所有节点的父节点，然后我们就可以利用节点的父节点信息从 p 结点开始不断往上跳，并记录已经访问过的节点，再从 q 节点开始不断往上跳，如果碰到已经访问过的节点，那么这个节点就是我们要找的最近公共祖先）。

15. （124）二叉树中的最大路径和（最终的结果不是递归的结果，而是递归的中间结果，思路计算左右子树的最大长度，然后和根节点做**加和**；具体可以查看相应的注释）。
...
2. （78）子集（递归，回溯模板解法，每一个结果都要保存；递归，选取 or 不选取，终止模板是 cur == n）。 
3. 电话号码的字母组合（递归，回溯模板解法，不要在循环内使用 String += String，直接使用 StringBuilder 就可）。 
4. 组合总和（递归，先排序做好剪枝，从 index 开始，防止回头式搜素导致组合重复，由于可以元素重复，递归时 index 传入 i）。 
5. 括号生成（递归，选取左括号 or 选取右括号，做好剪枝 left <= right，终止条件是左右括号数均为 0）。 
6. 单词搜索（递归，非回溯模板解法，但仍然是回溯，需要在回溯过程中适时终止【注意，BFS 本身做不到回溯】）。 
7. 分割回文串（递归，回溯模板解法，需要提取一个判断是否回文串的子函数）。
8. N 皇后（递归，回溯模板解法，**画出回溯树**，按行进行回溯，判断是否合法只能朴素判断，合法性判断只需要考虑上半部分，而不需要考虑下半部分）。
...
6. 寻找两个正序数组的中位数（尾递归，假设 k 是我们要找的中位数，那么我们可以在两个数组 A、B 中分别寻找第 k / 2 大的数，丢掉较小的哪一段，参考讲解 BV：BV1JL411p73u，详细看代码注释）。

## 12.栈（字符串编解码）

1. （20）有效的括号（使用栈去完成，最后的判别条件是栈是否为空，穷举括号类型即可）。
...
6. 柱状图中的最大矩形（双指针，有点类似于接雨水，使用数组存储左右的最远位置，然后计算最大矩形，详情看注释，注意时间复杂度是 O(n)，重点是 while 循环 + t 值非线性变换获取左右边界）。

## 13.堆（数组中第 k 个最大元素）

1. （215）数组中第 k 个最大元素（使用最大堆，可使用 Java PriorityQueue，但是也可以**自建最大堆**，自建堆使用数组，）。 
2. （347）前 k 个高频元素（使用最大堆，借助 Map 统计每一个 nums[i] 出现的次数，然后使用出现次数建立最大堆，`PriorityQueue` 可以传入比较函数）。
3. （295）数据流的中位数（结合最大堆和最小堆，左边大顶堆，右边小顶堆，偶数加小堆，奇数加大堆，先加给对家，然后拿其顶，奇数取大顶，偶数取除 2）。
